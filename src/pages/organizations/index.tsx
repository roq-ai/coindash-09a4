...
function OrganizationListPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<OrganizationInterface[]>(
    ...
    () =>
      getOrganizations({
        relations: ['user', 'crypto_marketing_service.count', 'service_plan.count'],
      }),
  );
              ...
              <Thead>
                <Tr>
                  ...
                  {hasAccess('crypto_marketing_service', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                    <Th>crypto_marketing_service</Th>
                  )}
              ...
              <Tbody>
                {data?.map((record) => (
                  <Tr cursor="pointer" onClick={() => handleView(record.id)} key={record.id}>
                    ...
                    {hasAccess('crypto_marketing_service', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                      <Td>{record?._count?.crypto_marketing_service}</Td>
                    )}
                    {hasAccess('service_plan', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                      <Td>{record?._count?.service_plan}</Td>
                    )}
                    <Td>
